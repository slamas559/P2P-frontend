...
import { FaUsers, FaExchangeAlt, FaUserShield, FaHistory } from "react-icons/fa";
...
const AdminDashboard = () => {
  ...
  const toggleAdmin = async (userId, currentRole) => {
    try {
      const endpoint = currentRole === 'dealer'
  ...
  useEffect(() => {
    ...
    const filtered = trades.filter(
      (trade) =>
        trade._id.toLowerCase().includes(lower) ||
        trade.crypto.toLowerCase().includes(lower) ||
        trade.type.toLowerCase().includes(lower) ||
        trade.paymentMethod.toLowerCase().includes(lower)
    );
                ...
                <td className="p-3">
                  {editingId === trade._id ? (
                    <input
                      ...
                      onChange={(e) =>
                        setEditForm({ ...editForm, crypto: e.target.value })
                      }
                  ...
                  {editingId === trade._id ? (
                    <>
                      <button
                        ...
                        className="bg-neon text-dark px-3 py-1 rounded"
                      >
                        Save
                      </button>
                      <button
